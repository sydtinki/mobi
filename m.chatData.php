...
if($mnun || $message!=""){confirmSend($message,$sender,$me,$mnun);

}
function confirmSend($c,$d,$e,$f){ 
$count="select count(message) from messages where message='$message' AND m_numb='$mnun'"; 
$result = mysql_query($count);  
$rows = mysql_num_rows($result); 

for($i=0; $i<$rows; ++$i){  
$row = mysql_fetch_row($result); 
doSend($row[0],$c,$d,$e,$f);
  
} 
}
function doSend($x,$c1,$d1,$e1,$f1){ 
     
if($x==0){ 
$stamp=date('Y m d h i s');
$time=time();  

$send="INSERT INTO messages VALUES('$f1','$c1','$d1', '$e1', '$stamp','$time', 'unread')";  
mysql_query($send);
 
}
}
if($rows>=8){ echo "<span id='LoadAll'>[ <a href='javascript:loadMore()'>Load Earlier Messages </a>]</span><p><b id='loading'>loading..</b><p>"; for($i=0; $i<$rows; $i++){ $idx++; 
        $row = mysql_fetch_row($result);        
